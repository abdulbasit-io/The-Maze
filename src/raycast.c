		...
		SDL_bool *map_active)
{
	...
	center.x = player->rect.x + (player->rect.w / 2);
	center.y = player->rect.y + (player->rect.h / 2);
		...
		double ray_view_angle)
{
	 ...
	 * To get the correct distance, multiply the distorted
	...
	SDL_Color wall_color)
{
	...
	double draw = ((SCREEN_HEIGHT - line_height) / 2.0);

	texture_walls(sdl, line_height, ray_index, draw, wall_color.a);
}
