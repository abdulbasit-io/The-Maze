...
void player_collision_detection(player *player, map_t *map)
{
	...
	int i, j;
	int border = 5;
	int dimensions = player->rect.w + border;
	SDL_Rect bounding_box = {player->rect.x - (border / 2),
		player->rect.y - (border / 2),
 ...
 * @wall: data structure  to check for intersection on sides
 * @pointA: Pointer at beginning of line
